...
public enum CoreQuerys {
	...
	ACCOUNT_INSERT("INSERT INTO `accounts` (`uniqueid`,`nickname`,`address`,`group`,`timegroup`,`joinin`,`lastsee`,`tag`,`permissions`) VALUES (?,?,?,?,?,?,?,?,?);"),
	ACCOUNT_UPDATE("UPDATE `accounts` SET `nickname`=?,`address`=?,`group`=?,`timegroup`=?,`lastsee`=?,`tag`=?,`permissions`=? WHERE `uniqueid`=?"),
	
	TAG_SELECT("SELECT * FROM `tags` WHERE `tagname`=?"),
	TAG_SELECT_ALL("SELECT * FROM `tags`;"),
	TAG_INSERT("INSERT INTO `tags` (`tagname`,`tagprefix`,`tagorder`,`tagexclusive`) VALUES (?,?,?,?);"),
	TAG_UPDATE("UPDATE `tags` SET `tagprefix`=?,`tagorder`=?,`tagexclusive`=? WHERE `tagname`=?"),
